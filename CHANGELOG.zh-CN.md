...
## [未发布] — 云端会话管理与日常工具 (2026-09-25)

### 新增
- **云端媒体工具**：`CloudClient` 增加类型化的消息、图片生成和 TTS 操作；云聊界面使用已配置 provider，provider 未配置时显示明确错误。

### 变更
- **依赖维护**：更新 Drizzle ORM/Kit 和 Vitest 补丁版本，显式补齐 Testing Library DOM peer；依赖审计从 1 high / 7 moderate 降至 0 high / 4 moderate，未进行不安全的降级。

### 新增
- **设备与会话管理**：已登录用户可以在设置页查看活跃会话、设备与活跃时间，并吊销其他会话。
- **聊天搜索**：聊天列表支持按会话名、好友名、中文名和好友备注过滤本地缓存，不新增后端请求。
- **今日日程**：已连接日历能力时，设置页可以展示今日事件；未连接时显示配置引导。
- **会话安全元数据**：会话保存 access token 哈希、User-Agent 摘要和最近活跃时间，吊销或过期后立即拒绝对应访问令牌。

### 变更
- **云端日历 API**：新增带授权校验的 `GET /v1/capabilities/calendar/today`，未连接时返回配置引导，已连接时写入工具执行审计。
- **开发环境访问**：Vite 开发服务器允许已配置的 Cloudflare 和 Serveo 隧道域名，用于本地及远程验收。
...
- **API 密钥改为会话级保存**（`src/config/apiConfig.js`、`src/components/ApiConfigPanel.jsx`）
  ...
  - 设置面板新增提示，明确 API 密钥仅在当前浏览器会话中保留
...
- **分层覆盖率门禁**（`vitest.config.js`、`vitest.full-coverage.config.js`、`docs/TEST_COVERAGE_ANALYSIS.md`）
  - 398/398 项测试通过；对回归关键逻辑保持 60% 硬门禁
