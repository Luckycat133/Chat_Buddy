...
### ☁️ 云端会话工具
- **活跃会话**：查看设备摘要、创建时间和最近活跃时间；可吊销当前会话之外的其他会话。
- **本地聊天搜索**：从聊天列表搜索入口按会话名、好友名或好友备注过滤本地缓存。
- **今日日程**：日历能力已连接时在设置页查看今日事件，否则按页面提示完成连接。
...
- 云端操作：基于 Postgres 的 Fastify 鉴权路由，支持活跃会话吊销和今日日程读取授权
